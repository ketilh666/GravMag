...
for ktest in range(0,ntest):
    ...
    model.z[1] = 10*model.z[1]
    model.mag[6,3] = model.mag[4,6].copy()
    model.mag[2,9] = model.mag[4,6].copy()
    model.mag[4,6] = 0.0
...
#CUT            pickle.dump([data, model], fid) 
    ...
    zz = model.z[1][0,0]-model.z[0][0,0]
    fig.suptitle('Test {}: dx=dy={}, z2-z1={}, zr={}'.format(ktest, gscl, zz, zr), fontsize=14)
